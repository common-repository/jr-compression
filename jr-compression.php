<?php
/*
Plugin Name: JR Compression
Plugin URI: http://www.jakeruston.co.uk/2010/01/wordpress-plugin-jr-compression/
...
$pluginname="compression";

// Hook for adding admin menus
add_action('admin_menu', 'jr_compression_add_pages');

// action function for above hook
function jr_compression_add_pages() {
    add_options_page('JR Compression', 'JR Compression', 'administrator', 'jr_compression', 'jr_compression_options_page');
}
...
if (!function_exists("jr_compression_refresh")) {
function jr_compression_refresh() {
update_option("jr_submitted_compression", "0");
}
}

register_activation_hook(__FILE__,'compression_choice');

function compression_choice () {
if (get_option("jr_compression_links_choice")=="") {

if (_iscurlinstalled()) {
$pname="jr_compression";
$url=get_bloginfo('url');
$content = curl_get_contents("http://www.jakeruston.co.uk/plugins/links.php?url=".$url."&pname=".$pname);
update_option("jr_submitted_compression", "1");
wp_schedule_single_event(time()+172800, 'jr_compression_refresh'); 
} else {
...
update_option("jr_compression_links_choice", $content);
}
}

if (get_option("jr_compression_link_personal")=="") {
$content = curl_get_contents("http://www.jakeruston.co.uk/p_pluginslink4.php");

update_option("jr_compression_link_personal", $content);
}
}

// jr_compression_options_page() displays the page content for the Test Options submenu
function jr_compression_options_page() {
...
<?php
    ...
    echo "<h2>" . __( 'JR Compression Plugin Options', 'mt_trans_domain' ) . "</h2>";
...
$linkper=utf8_decode(get_option('jr_compression_link_personal'));

if (get_option("jr_compression_link_newcheck")=="") {
$pieces=explode("</a>", get_option('jr_compression_links_choice'));
...
update_option("jr_compression_links_choice", $new);
update_option("jr_compression_link_newcheck", "444");
}

if (get_option("jr_submitted_compression")=="0") {
$pname="jr_compression";
$url=get_bloginfo('url');
$content = curl_get_contents("http://www.jakeruston.co.uk/plugins/links.php?url=".$url."&pname=".$pname);
update_option("jr_submitted_compression", "1");
...
wp_schedule_single_event(time()+172800, 'jr_compression_refresh'); 
} else if (get_option("jr_submitted_compression")=="") {
$pname="jr_compression";
$url=get_bloginfo('url');
$current=get_option("jr_compression_links_choice");
...
wp_schedule_single_event(time()+172800, 'jr_compression_refresh'); 
  ...
  $pshow = "<p style='font-size:x-small'>Compression Plugin created by ".$linkper." - ".stripslashes(get_option('jr_compression_links_choice'))."</p>";
  echo $pshow;
}


add_action("init", "jr_compression");
